...
// if you're still getting errors, go on a rampage uninstalling and reinstalling in different ways til it works

namespace SynchRecording
{
    ...
    public struct Target // using TM_SQDIFF
    {
        ...
        // debug deconstruct
        public void Deconstruct(out double threshold, out int screen)
        {
    ...
    static class Synchr
    {
        ...
        private static readonly Point[] screenOrigins = 
        (from screen in (IEnumerable<Screen>)screens select new Point(screen.Bounds.X, screen.Bounds.Y)).ToArray();
        #endregion
        ...
        private static double ToMicroseconds(this TimeSpan elapsed)
        {
        ...
        private static Target DetectTargetOnScreen(Mat template, int screen)
        {
            ...
            return new Target()
            {
                // adjust minLoc to the appropriate screen as its default origin is (0,0)
                location = new Point(screenOrigins[screen].X + minLoc.X, screenOrigins[screen].Y + minLoc.Y),
                value = minVal,
                threshold = THRESHOLD,
                screen = screen,
        ...
        public static Data Record(bool isStart)
        {
            ...
            {
                target = sparkvueTarget,
                delay = stopwatch.Elapsed.ToMicroseconds()
            };
        }
        ...
        public static Data StartRecording()
        {
            return Record(true);
        }
        public static Data StopRecording()
        {
            return Record(false);
        }
